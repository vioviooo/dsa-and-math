...
// * infix iter operator region start
...
bool b_tree<tkey, tvalue>::infix_iterator::operator==(
    b_tree::infix_iterator const &other) const noexcept
{
    if (_path.empty() and other._path.empty()) {
        return true;
    }
...
{
    if (_path.empty()) {
        std::logic_error("undefined behavior");
        // return *this;
    }
    ...
    while (!_path.empty() and index == node->virtual_size - (node->subtrees[0] == nullptr
        ? 1
        : 0));
...
// * infix iter operator region end
...
// * infix const iter operator region start
...
// * infix const iter operator region end
    ...
    typename associative_container<tkey, tvalue>::key_value_pair &&kvp)
{
    ...
    if (node == nullptr and path.size() == 1) {
    ...
    bool upper_bound_inclusive)
{
    ...
    // * find the starting point that meets the lower bound condition
    while ((it != this->end_infix()) and (this->_keys_comparer(std::get<2>(*it), lower_bound) < (lower_bound_inclusive ? 0 : 1))) {
        ++it;
    }

    // * add all elems that meet upper bound condition
    while ((it != this->end_infix()) and (this->_keys_comparer(upper_bound, std::get<2>(*it)) > (upper_bound_inclusive ? -1 : 0))) {
        result.push_back(typename associative_container<tkey, tvalue>::key_value_pair(std::get<2>(*it), std::get<3>(*it)));
        ++it;
    }
...
template<
    typename tkey,
    typename tvalue>
inline size_t b_tree<tkey, tvalue>::get_min_keys_count() const noexcept // * min amount of keys in node
{
    return _t - 1;
}
...
template<
    typename tkey,
    typename tvalue>
inline size_t b_tree<tkey, tvalue>::get_max_keys_count() const noexcept // * max amount of keys in node
{
    return 2 * _t - 1;
}
