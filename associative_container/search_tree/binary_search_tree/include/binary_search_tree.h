    ...
    tvalue const &value)
{
    ...
    *(path.top()) = reinterpret_cast<node *>(allocate_with_guard(get_node_size(), 1));
    call_node_constructor(*(path.top()), key, value);
    balance(path);
}
    ...
    tvalue &&value)
{
    ...
    *(path.top()) = reinterpret_cast<node *>(allocate_with_guard(get_node_size(), 1));
    call_node_constructor(*(path.top()), key, std::move(value));
    balance(path);
}
